...
_RTMLIB_NOTE = ("rtmlib downloads the ONNX models on first use (download.openmmlab.com / "
                "huggingface.co) into its cache folder.")


def _rtm(key, label, factory, fmt, notes, modes=_RTMLIB_MODES, provides_3d=False) -> BackendSpec:
    return BackendSpec(
        key=key, label=label, module="poseboard.pose.detectors.rtmlib_det", factory=factory,
        requires=_RTMLIB, install=_RTMLIB_INSTALL,
        options={"mode": modes, "device": _DEVICE},
        defaults={"mode": modes[0], "device": "auto"},
        keypoint_format=fmt, license="Apache-2.0", notes=notes + " " + _RTMLIB_NOTE,
        provides_3d=provides_3d)
...
BACKENDS: dict[str, BackendSpec] = {s.key: s for s in (
    BackendSpec(
        ...
        notes="Built in. Also gives a metric 3D skeleton, so one camera is enough for 3D "
              "(depth approximate). num_poses > 1 lets PoseBoard pick the person on the board.",
        provides_3d=True),
    ...
    BackendSpec(
        ...
        options={"device": _DEVICE}, defaults={"device": "auto"},
        keypoint_format="coco17", license="BSD-3-Clause",
        notes="Weights download from download.pytorch.org on first use. Slow without a GPU."),
    BackendSpec(
        key="vitpose_hf", label="ViTPose (Hugging Face transformers)",
        module="poseboard.pose.detectors.vitpose_hf_det", factory="create",
        requires=("transformers", "torch"), install="pip install transformers torch",
        options={"model": ("usyd-community/vitpose-base-simple", "usyd-community/vitpose-plus-small",
                 ...
                 "device": _DEVICE},
        defaults={"model": "usyd-community/vitpose-base-simple", "device": "auto"},
        keypoint_format="coco17", license="Apache-2.0",
        notes="RT-DETR person detector + ViTPose. Models download from huggingface.co on first "
              "use. Slow without a GPU."),
    BackendSpec(
        ...
        options={"model": ("body25", "coco18"), "input_size": (368, 256, 480, 656),
                 "prototxt": (), "caffemodel": ()},
        defaults={"model": "body25", "input_size": 368, "prototxt": "", "caffemodel": ""},
        keypoint_format="body25", license="OpenPose license: academic / non-commercial only",
        notes="Single person. Slow on CPU. The model files are not downloaded automatically.",
    ...
    BackendSpec(
        ...
        keypoint_format="coco17", license="Apache-2.0",
        notes="Any MMPose 2D model (config name or alias); the keypoint layout depends on the "
              "model (COCO-17 by default). Weights download from download.openmmlab.com."),
    BackendSpec(
        ...
        options={"model": ("lightning", "thunder"), "model_path": ()},
        defaults={"model": "lightning", "model_path": ""},
        keypoint_format="coco17", license="Apache-2.0",
        notes="Very fast, single person. Select a MoveNet .onnx file (model_path) if it cannot "
              "be downloaded.",
